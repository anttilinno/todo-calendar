...
**internal/app/keys.go changes:**
Update the status bar help text. In `ShortHelp()` and `FullHelp()`, the existing Quit and Tab bindings remain. No structural changes needed to keys.go itself, BUT update the status bar in `model.go` View():

...
must_haves:
  ...
  artifacts:
    ...
    - path: "internal/todolist/model.go"
      provides: "dateLayout and datePlaceholder fields, SetDateFormat setter, format-aware rendering and input parsing"
...
<task type="auto">
  ...
  <action>
**settings/model.go:**
1. Add `"time"` import (fmt already present).
...
Add import for `"github.com/antti/todo-calendar/internal/config"` -- todolist imports config for `config.FormatDate` and `config.ParseUserDate` calls below.

1. Add `dateLayout string` and `datePlaceholder string` fields to the Model struct.
2. In `New()`, initialize both fields: `dateLayout: "2006-01-02"` (ISO default) and `datePlaceholder: "YYYY-MM-DD"` (ISO default).
3. Add `SetDateFormat(layout, placeholder string)` setter method on `*Model`:
   ```go
   ...
   ```
   The app layer calls `tl.SetDateFormat(cfg.DateLayout(), cfg.DatePlaceholder())` to pass both values.
4. In `renderTodo()`, change `m.styles.Date.Render(t.Date)` to `m.styles.Date.Render(config.FormatDate(t.Date, m.dateLayout))`.
5. In `updateNormalMode()` for AddDated (key.Matches(msg, m.keys.AddDated) block), after entering dateInputMode: change `m.input.Placeholder = "YYYY-MM-DD"` to `m.input.Placeholder = m.datePlaceholder`.
6. In `updateDateInputMode()` Confirm handler: replace `time.Parse("2006-01-02", date)` validation with `config.ParseUserDate(date, m.dateLayout)` and use the returned ISO string for `m.store.Add(m.pendingText, isoDate)`.
7. In `updateNormalMode()` EditDate handler: change `m.input.Placeholder = "YYYY-MM-DD (empty = floating)"` to `m.input.Placeholder = m.datePlaceholder + " (empty = floating)"`. Change `m.input.SetValue(todo.Date)` to `m.input.SetValue(config.FormatDate(todo.Date, m.dateLayout))` so the pre-populated value matches the display format.
8. In `updateEditDateMode()` Confirm handler: replace `time.Parse("2006-01-02", date)` validation with `config.ParseUserDate(date, m.dateLayout)` and use the returned ISO string for `m.store.Update(m.editingID, todo.Text, isoDate)`.

**app/model.go:**
1. In `New()`, after creating `tl := todolist.New(s, t)`, add: `tl.SetDateFormat(cfg.DateLayout(), cfg.DatePlaceholder())`.
2. In the `settings.SaveMsg` handler, after `m.calendar.SetMondayStart(msg.Cfg.MondayStart())`, add: `m.todoList.SetDateFormat(m.cfg.DateLayout(), m.cfg.DatePlaceholder())`. NOTE: `m.cfg` is already updated from `msg.Cfg` earlier in the SaveMsg handler (the existing code does `m.cfg = msg.Cfg` before the SetMondayStart call), so `m.cfg.DateLayout()` and `m.cfg.DatePlaceholder()` correctly reflect the newly saved config.

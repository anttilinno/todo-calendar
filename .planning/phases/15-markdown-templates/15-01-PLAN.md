...
2. In `internal/store/sqlite.go`:
   ...
   - Update `Add()` method: change the INSERT SQL to include the body column with a hardcoded empty string `''` (e.g., `INSERT INTO todos (text, body, date, done, created_at, sort_order) VALUES (?, '', ?, ?, ?, ?)`). The Go method signature stays `Add(text string, date string) Todo` -- body is always empty on initial creation; the template flow uses UpdateBody() separately.

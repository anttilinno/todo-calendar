...
2. In `internal/todolist/model.go`:
   - Import `"github.com/antti/todo-calendar/internal/tmpl"`.
   - Add new modes to the `mode` enum (after `filterMode`):
     ...
     - `templateContentMode` -- entering content for a new template
...
4. Add `updateTemplateNameMode(msg tea.KeyMsg) (Model, tea.Cmd)`:
   - On Enter:
     ...
     - Configure input: `m.input.Placeholder = "Template content (use {{.VarName}} for placeholders)"`, `m.input.Prompt = "> "`, `m.input.SetValue("")`.
   - On Esc: cancel, return to normalMode.
   - Otherwise: forward to text input.

5. Add `updateTemplateContentMode(msg tea.KeyMsg) (Model, tea.Cmd)`:
   - On Enter:
     - `content := strings.TrimSpace(m.input.Value())`. If empty, stay.
     - Call `m.store.AddTemplate(m.pendingTemplateName, content)`.
     - Return to normalMode. Clear input and template state.
   - On Esc: cancel, return to normalMode.
   - Otherwise: forward to text input.
...
9. In `Update()`, add routing for the new modes in the KeyMsg switch:
   - `templateSelectMode`: call `m.updateTemplateSelectMode(msg)`
   - `placeholderInputMode`: call `m.updatePlaceholderInputMode(msg)`
   - `templateNameMode`: call `m.updateTemplateNameMode(msg)`
   - `templateContentMode`: call `m.updateTemplateContentMode(msg)`

10. In `View()`, add rendering for `templateSelectMode`:
    - Show a header "Select Template:" styled like section headers.
    - List templates with cursor indicator ("> " for selected, "  " for others).
    - Show template name and a truncated preview of content.
    - Show hint: "enter select | d delete | esc cancel".
